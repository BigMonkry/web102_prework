...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        const thegame = games[i];
        gameCard.innerHTML = `
        <img src="${thegame.img}" alt="game" class="game-img">
        <h1>${thegame.name}</h1>
        <p>Description: ${thegame.description}</p>
        <p>Backers: ${thegame.backers}</p>
        <p>Pledged: ${thegame.pledged}</p>
    `;
...
const totalContributions = GAMES_JSON.reduce( (acc, thegame) => {
    return acc + thegame.backers;
  }, 0);
...
const totalpledged = GAMES_JSON.reduce( (acc, thegame) => {
    return  acc + thegame.pledged;
  }, 0);
...
const displayStr =
  unfundedGamesCount === 1
    ? `A total of $${totalpledged.toLocaleString()} has been raised for ${
        GAMES_JSON.length
      } games. Currently, ${unfundedGamesCount} game remains unfunded. We need your help to fund these amazing games!`
    : `A total of $${totalpledged.toLocaleString()} has been raised for ${
        GAMES_JSON.length
      } games. Currently, ${unfundedGamesCount} games remain unfunded. We need your help to fund these amazing games!`;
...
<img src="${firstGame.img}" alt="game" class="game-img"> 
`;
...
<img src="${secondGame.img}" alt="game" class="game-img">`;
